...
| <a id="verilog_dv_tb-vcs_three_step"></a>vcs_three_step |  Use VCS three-step analysis/elaboration. Dependencies selected by `vcs_vlogan_precompile_deps` form a frozen prefix before the remaining project filelists in one `vlogan -incr_vlogan` command. When enabled, `extra_compile_args` must be empty; put analysis options in `vcs_vlogan_args` and elaboration options in `vcs_elab_args`.   | Boolean | optional | `False` |
...
| <a id="verilog_dv_tb-vcs_vlogan_precompile_deps"></a>vcs_vlogan_precompile_deps |  Dependencies whose transitive VCS filelists form a frozen prefix in the single incremental `vlogan` analysis command. Each selected dependency must already be reachable through `deps` or `shells`. The remaining project filelists follow the prefix without changing their internal order.   | <a href="https://bazel.build/docs/build-ref.html#labels">List of labels</a> | optional | [] |

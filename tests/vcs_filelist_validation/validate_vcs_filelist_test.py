...
class VcsFilelistValidationTest(unittest.TestCase):
    ...
    def test_unit_test_scripts_select_the_requested_backend(self):
        ...
        self.assertIn("+warn=all", rtl_compile_args)
        self.assertIn("-timescale=100fs/100fs", rtl_compile_args)
        self.assertIn("+define+TIMESCALE_STEP_FS=100\n", rtl_compile_args)
        self.assertIn("+define+TIMESCALE_PREC_FS=100\n", rtl_compile_args)
        self.assertNotIn("+DV_LEGACY_RUNTIME", dv_compile_args)
...
"""
        ...
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            self.assertTrue(any("+DV_LEGACY_RUNTIME" in record["args"] for record in runtime_records))
            for record in runtime_records:
                for unwanted in ("-input", "ignored_runtime_waves.tcl", "-access", "+rw", "-debug_opts", "verisium_pp"):
                    self.assertNotIn(unwanted, record["args"])

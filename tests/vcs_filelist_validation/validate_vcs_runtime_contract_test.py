...
"""
    ...
    def test_vcs_three_step_template_uses_one_incremental_analysis_context(self):
        ...
        analysis = calls[0].splitlines()
        elaboration = calls[1].splitlines()
        self.assertEqual("vlogan", analysis[0])
        self.assertIn(shell_path(uvm_pkg), analysis)
        self.assertIn("-incr_vlogan", analysis)
        self.assertIn("-vts_ignore_env=HOSTNAME,LSB_JOBID", analysis)
        self.assertIn("-sverilog", analysis)
        self.assertIn("+incdir+vip/includes", analysis)
        self.assertIn("+incdir+project/includes", analysis)
        self.assertEqual(2, analysis.count("-file"))
        self.assertIn("vip.f", analysis)
        self.assertIn("project.f", analysis)
        self.assertLess(analysis.index(shell_path(uvm_pkg)), analysis.index("vip.f"))
        self.assertLess(analysis.index("vip.f"), analysis.index("project.f"))
        self.assertIn("+define+PROJECT_DEFINE", analysis)
        self.assertNotIn(shell_path(vlogan_args), analysis)

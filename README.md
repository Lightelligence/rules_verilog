...
All standard VCS compile paths use `-debug_access+r+w+f`, providing signal
read/write/force access required by UVM HDL deposit/force calls and DUT memory
backdoor loads without requiring waves. Wave builds additionally enable KDB and
FSDB capture; they do not enable VPI or UVM transaction recording.
...
cross-file macro state inside each group. Before the project group, the flow
replays top-level `.vh` and `.svh` entries from frozen filelists so generated
RTL wrapper macros remain available without editing RTL sources. Other macro
state does not cross the group boundary, so downstream sources must import
compiled packages rather than depend on nested include-guard side effects from
the frozen group. Analysis and elaboration arguments must be split between
`vcs_vlogan_args` and `vcs_elab_args`; see the

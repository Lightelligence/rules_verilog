...
`vlogan -incr_vlogan` command with the dependencies selected by
`vcs_vlogan_precompile_deps` ordered before the remaining project filelists.
Keeping one analysis command preserves cross-file macro and include-guard
state, while the stable frozen prefix lets incremental analysis avoid repeating
unchanged VIP work after a project edit. Analysis and elaboration arguments
